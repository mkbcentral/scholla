...
class PaimentPrinterConteroller extends Controller
{
    public function printRecuPaiementInscription($inscriptions)
    {
        ...
        $pdf->loadView(
            'pages.paiement.pints.print-recu-inscription',
    ...
    public function printRecuFrais($paiement)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-recu-paiment',
            compact(['paiement', 'taux'])
        );
        return $pdf->stream();
    }
    ...
    public function printRapportInscJournlaier($date)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-paiment-insc-journ',
    ...
    public function printRapportInscAll($status, $dateTo, $dateFrom, $type, $classe_id)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-all-paiment-insc',
    ...
    public function printRapportInscMonth($month)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-paiment-insc-mounth',
    ...
    public function printRapportInscPeriode($periode)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-paiment-insc-periode',
    ...
    public function printRapportPaiemenFraisDay($date, $cost_id, $month, $type, $classe_id, $idScolaryYer)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-paiement-frais-day',
    ...
    public function printRapportPaiemenFraisMonth($month, $cost_id, $type, $classeId, $idScolaryYer)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-paiement-frais-month',
    ...
    public function printRapportPaiemenFraisPeriode($periode, $cost_id, $month, $type, $classe_id, $idScolaryYer)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-paiement-frais-periode',
    ...
    public function printRapportGlobalFrais($type, $cost_id, $paiement_type, $classe_id, $idScolaryYer)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-global-frais',
    ...
    public function printFraisEtat($classeId, $costId)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-frais-etat',
    ...
    public function printFraisEtatByDate($date)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-frais-etat-by-date',
    ...
    public function printFraisEtatBySection($sectionId, $costId)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-frais-etat-soection',
    ...
    public function printRapportAnnuel($sectionId, $typeId,$optionId)
    {
        $taux = 2000;
        $section = Section::find($sectionId);
        $defaultScolaryYer = ScolaryYear::where('active', true)->first();
        $myOption=ClasseOption::find($optionId);
        $typeCost=TypeOtherCost::find($typeId);
        if($optionId==0){
            $paiments = Paiment::select('paiments.*',)
            ->join('cost_generals', 'cost_generals.id', '=', 'paiments.cost_general_id')
            ->join('type_other_costs', 'type_other_costs.id', '=', 'cost_generals.type_other_cost_id')
            ...
            ->join('classe_options', 'classe_options.id', '=', 'classes.classe_option_id')
            ->join('sections', 'sections.id', '=', 'classe_options.section_id')
            ->where('cost_generals.type_other_cost_id', $typeId)
            ->where('sections.id', $sectionId)
            ->where('paiments.scolary_year_id', $defaultScolaryYer->id)
            ->with('cost')
            ->with('student')
            ...
            ->where('cost_generals.type_other_cost_id', $typeId)
            ->where('sections.id', $sectionId)
            ->where('classe_options.id', $optionId)
            ->where('paiments.scolary_year_id', $defaultScolaryYer->id)
            ->with('cost')
            ->with('student')
            ->with('student.classe')
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-annuel',
            compact(['defaultScolaryYer', 'paiments', 'section', 'taux','myOption','typeCost'])
        )
            ->setPaper('a4', 'landscape')->setWarnings(false)->save('rapport.pdf');
        return $pdf->stream();
    }
    ...
    public function printArchive($classeId, $costId, $month)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-frais-archive',
    ...
    public function printArchiveGlobal($costId, $month)
    {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView(
            'pages.paiement.pints.print-rapport-frais-archive-global',
