...
<body>
    <div style="border: 1px solid black;padding: 6px">
        <div style="text-align: center;padding: 15px;font-size: 22px">
            <span>SITUATION DES DEPOTS A LA BANQUE</span>
        </div>
        <div>
            <span style="margin-top: 8px;margin-bottom: 8px ">Mois de: {{strftime('%B', mktime(0, 0, 0, $month))}}</span>
        </div>
        <div>
            <span style="margin-top: 8px;margin-bottom: 8px ">Abnnée scolaire: {{$defaultScolaryYer->name}}</span>
        </div>
    </div>
    </div>
        ...
        @if ($depots->isEmpty())
                    ...
                    <table sty>
                        ...
                        @foreach ($depots as $index => $depot)
                                <tr>
                                    <td>{{$index+1}}</td>
                                    <td>{{$depot->created_at->format('d/m/Y')}}</td>
                                    <td style="">{{$depot->title}}</td>
                                    <td style="text-align: right">{{number_format($depot->amount,1,',',' ')}}</td>
                                    <td style="text-align: right">{{$depot->observation}}</td>
                                </tr>
                                @php
                                    $total+=$depot->amount;
                                @endphp
...
<div style="margin-top: 8px">
    ...
    <div style="font-size: 18px;margin-top: 20px; padding: 8px;color: rgb(32, 32, 32)">
        <span style="font-weight: bold;margin-right: 700px">COMPTABLE</span>
