...
<body>
    <div style="border: 1px solid black;padding: 6px">
        <div style="text-align: center;padding: 15px;font-size: 22px">
            <span>SITUATION DES DEPENSES MENSUELLES</span>
        </div>
        <span style="margin-top: 8px;margin-bottom: 8px ">Du moi de: {{strftime('%B', mktime(0, 0, 0, $month))}}</span>
    </div>
    </div>
        ...
        @if ($depenses->isEmpty())
                    ...
                    <table sty>
                        ...
                        @foreach ($depenses as $depenses)
                                <tr>
                                    <td>{{$depenses->created_at->format('d/m/Y')}}</td>
                                    <td style="">{{$depenses->title}}</td>
                                    <td style="text-align: right">{{number_format($depenses->amount,1,',',' ')}}</td>
                                    <td style="text-align: right">{{$depenses->description}}</td>
                                </tr>
                                @php
