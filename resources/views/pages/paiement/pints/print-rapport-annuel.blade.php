...
<body>
    <div style="border: 1px solid black;padding: 6px">
        <div style="text-align: center;padding: 15px;font-size: 22px">
            <span style="text-transform: uppercase">RAPPORT DE PAIMENT FRAIS DE L'ETAT PAR SECTION</span>
        </div>
        <div>
            <span><b>Section</b>: {{ $section->name }}{{ $myOption != null ? '/' . $myOption->name : '' }}</span>
        </div>
        <div>
            <span><b>Type frais</b>: {{ $typeCost->name }}</span>
        </div>
        <div>
            <span><b>Année scolaire</b>: {{ $defaultScolaryYer->name }}</span>
        </div>
    </div>
    </div>
        ...
        <table sty>
            <thead style="background: rgb(67, 67, 67);color: rgb(222, 221, 221)">
                <tr>
                    ...
                    <th style="text-align: left">DATE PAIEMENT</th>
            ...
            <tbody>
                @foreach ($paiments as $index => $paiment)
                    <tr>
                        ...
                        <td>{{ $paiment->student->classe->name . '/' . $paiment->student->classe->option->name }}</td>
